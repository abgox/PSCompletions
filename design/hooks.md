...
- **Targets — validate against the manifest before adding**: every `command`/`option` in a spec must exist in `completions/<cmd>/language/en-US.json` and be a location that actually takes a runtime value — commands need a positional placeholder (`usage` with `<...>`/`[...]` or a free-form position), options need `next: []`/`next: [...]` (value-taking). **Match the value kind to the slot**: judge by what the CLI itself accepts there — its `--help` usage, docs, and examples — not by the manifest alone. File/dir candidates (`psc.glob`/`psc.ls`) belong at command positionals that take paths and at option value slots (`--config` with `next: []`); a filtered glob matching the command's recommended kinds (e.g. test files for a test runner) is encouraged — it finds deep files faster than manual browsing. What is forbidden is injecting files where the CLI takes something else: never offer config/profile files at a context whose slot takes subcommands, names, keys, or nothing (`{}`, `{ command = "build" }` offering `rspress.config.ts` where only `build`/`preview` are valid). If a slot accepts files but the manifest shows no placeholder, add the `usage` placeholder (`[FILES]...`) so the slot is documented. Once the user types a path separator (`./`, `../`, `/`, `C:\` …), the pending token routes to native filesystem completion instead of the manifest menu (`PSCompletions.ps1` dispatch), so hooks need not cover general browsing — their job is zero-typing discovery: surfacing the exact files a slot expects (the project's config file, the runner's test files in deep directories) before any path is typed.
