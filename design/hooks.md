...
| --- | --- | --- |
| `psc.items(elements, symbol_or_fn?)` | `psc_item[]` | Convert each element into a completion item. The second argument has three forms: **omitted** — the element itself is the `name` (must be a string; other types are skipped). **String** (`"stay"` or `"switch"`) — sets the symbol on every item. **Function** — `fn(elem)` returns the item table; returning `nil` skips that element. |
