Register-ArgumentCompleter -CommandName $PSCompletions.comp_cmd.pip -ScriptBlock {
    param($word_to_complete, $command_ast, $cursor_position)

    #region : Store
    $root_cmd = $PSCompletions.comp_cmd.pip
    ...
    $need_skip = @(
        '--help', '--format', '--editor', '--index',
        '--requirement', '--constraint', '--editable', '--target',
        '--platform', '--python-version', '--implementation', '--abi', '--root', '--prefix',
        '--src', '--upgrade-strategy', '--config-settings', '--install-option', '--global-option',
        '--no-binary', '--only-binary', '--progress-bar', '--root-user-action', '--report',
        '--index-url', '--extra-index-url', '--find-links', '--dest', '--exclude',
        '--wheel-dir', '--algorithm', '--build-option'
    )
    #endregion
