...
Register-ArgumentCompleter -CommandName $_psc.comp_cmd.yarn -ScriptBlock {
    param($wordToComplete, $commandAst)

    #region : Store
    $root_cmd = $_psc.comp_cmd.yarn
    $_i = 9999
    if (!$_psc.comp_data.$root_cmd) {
        ...
        $_psc.comp_data.$($root_cmd + '_info') = @{
            core_info = $json.yarn_core_info
            exclude   = @('yarn_core_info')
            num       = -1
        }
