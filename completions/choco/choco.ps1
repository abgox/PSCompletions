Register-ArgumentCompleter -CommandName $PSCompletions.comp_cmd.choco -ScriptBlock {
    param($word_to_complete, $command_ast, $cursor_position)

    #region : Store
    $root_cmd = $PSCompletions.comp_cmd.choco
    ...
    $filter_list = $completions.Keys | Where-Object {
        ...
        $cmd.Count -eq ($input_arr.Count + $space_tab) -and $cmd_str -like ($input_str + $match)
