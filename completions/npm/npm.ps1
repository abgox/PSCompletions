...
Register-ArgumentCompleter -CommandName $_psc.comp_cmd.npm -ScriptBlock {
    ...
    if (!$_psc.comp_data.$root_cmd) {
        ...
        $_psc.comp_data.$($root_cmd + '_info') = @{
            core_info = $json.npm_core_info
            exclude   = @('npm_core_info')
            num       = -1
        }
        ...
        $json.PSObject.Properties.Name | Where-Object {
            ...
            $_psc.comp_data.$root_cmd[$root_cmd + ' help ' + $cmd[0] ] = @($cmd[0], ($json.npm_core_info.help + ' --- ' + $cmd[0]), $_o)
