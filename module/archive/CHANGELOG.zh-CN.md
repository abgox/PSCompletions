...
- 新增 `enable_filter_subsequence_match` 菜单配置项
  ...
  - 启用后，过滤区域中的字符将按顺序匹配补全项中的字符，不要求连续出现，同时支持使用 `^` 符号进行前缀匹配
...
- 为 `$PSCompletions.argc_completions()` 添加了别名支持
  - 参考: [结合 argc-completions 使用](https://pscompletions.abgox.com/docs/tools/argc-completions)
...
- 移除了补全项中可能存在的 ANSI 转义码
  ...
  - [PSCompletions 可以正常和 Carapace 结合使用](https://pscompletions.abgox.com/docs/tools/carapace)
- 优化了命令及别名检查
- 其他的优化和修复
...
- 添加方法 `$PSCompletions.argc_completions()` 用于 [argc-completions](https://github.com/sigoden/argc-completions)。
  - 详情请查看: [PSCompletions and argc-completions](https://pscompletions.abgox.com/docs/tools/argc-completions)
