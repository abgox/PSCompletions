...
# Native-fallback ranking (TabExpansion2 items) via the engine's --sort mode; also hands the
# menu-order metadata to the menu process. Ranked commands run through the menu's build mode.
Add-Member -InputObject $PSCompletions -MemberType ScriptMethod apply_completions_sort {
...
Add-Member -InputObject $PSCompletions -MemberType ScriptMethod set_menu_order {
    param([string]$cmd)
...
Add-Member -InputObject $PSCompletions -MemberType ScriptMethod run_sort {
...
Add-Member -InputObject $PSCompletions -MemberType ScriptMethod write_binary_error {
...
Add-Member -InputObject $PSCompletions -MemberType ScriptMethod init_data {
    ...
    if ('psc' -notin $PSCompletions.data.list -and $PSCompletions.init_reentry -lt 2) {
        $PSCompletions.init_reentry = [int]$PSCompletions.init_reentry + 1
        & $pscBinary --data $dataDir add psc 2>$null | Out-Null
...
Add-Member -InputObject $PSCompletions -MemberType ScriptMethod psc_binary {
...
Add-Member -InputObject $PSCompletions.menu -MemberType ScriptMethod menu_binary {
